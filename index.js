...
const gamePatternRepeatTimeout = 800;


// Event listener for key presses.
$(document).keypress(function () {
    if (!bStarted) { // Checks if the 'bStarted' variable is at it's default state.

        $("#level-title").text("Level " + level); // If the 'started' variable is in it's default state. Then this line will change the text of the 'h1' element to "Level" + the value of the 'level' variable.
        nextSequence(); // Calls 'nextSequence()' function.
        bStarted = true; // Changes the 'bStarted' value to 'true'
    } // the 'else' statement is missing. Because if the value of 'started' isn't at default value (false). Then the game has started and key presses can be ignored.
});
